...
func (r DecisionRepositoryImpl) CreateOrUpdateDecision(ctx context.Context, actorID, recipientID string, liked bool) (bool, error) {
	...
	query := `INSERT INTO user_decisions (actor_id, recipient_id, liked, created_at, updated_at)
              VALUES (?, ?, ?, NOW(), NOW())
              ON DUPLICATE KEY UPDATE liked = ?, updated_at = NOW()`

	if _, err = tx.ExecContext(ctx, query, actorID, recipientID, liked, liked); err != nil {
		return false, fmt.Errorf("failed to put decision: %w", err)
	}

	// Only check for mutual like if this decision is a like
	mutualLike := false
	if liked {
		...
		if err = tx.QueryRowContext(ctx, checkQuery, recipientID, actorID).Scan(&mutualLike); err != nil {
